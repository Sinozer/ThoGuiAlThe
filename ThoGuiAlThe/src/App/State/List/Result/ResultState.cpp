...
void ResultState::InitUi()
{
	...
	if (I(NetworkManager).ReceiveData(TgatServerMessage::GAME_END, data) == true)
	{
		...
		auto* menu = m_UiManager.AddTextButton("MENU", "MENU", [this]()
			{
				I(NetworkManager).SendData(
					{
		...
		auto* exit = m_UiManager.AddTextButton("EXIT", "EXIT", [this]()
			{
				I(NetworkManager).SendData(
					{
						{ JSON_EVENT_TYPE, TgatClientMessage::LEAVE_SESSION },
				...
				StateManager::GetInstance()->RemoveAllStates();
			});
...
void ResultState::Update(const float& dt)
{
	...
	if (networkManager.ReceiveData(TgatServerMessage::GAME_REPLAY, data) == true)
	{
	...
	if (networkManager.ReceiveData(TgatServerMessage::SESSION_LEFT, data) == true)
	{
		...
		{
			m_UiManager.GetTextButton("REPLAY_BUTTON")->SetActive(false);
			m_UiManager.GetText("REPLAY_TEXT")->SetActive(false);
		}
	}
}
